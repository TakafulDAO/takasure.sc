...
contract SaveInvestCCIPReceiverForkTest is Test, GetContractAddress {
    ...
    function testSaveInvestCCIP_receiver_processesSaveVaultMessageOnArbitrumFork() public {
        uint256 amount = 25e6;
        bytes32 messageId = keccak256("SAVE_SUCCESS");

        usdc.mint(address(receiver), amount);
        Client.Any2EVMMessage memory message = _buildValidMessage(messageId, SAVE_VAULT, amount, user, allowedSender);
    ...
    function testSaveInvestCCIP_receiver_processesInvestVaultMessageOnArbitrumFork() public {
        uint256 amount = 40e6;
        bytes32 messageId = keccak256("INVEST_SUCCESS");

        usdc.mint(address(receiver), amount);
        Client.Any2EVMMessage memory message = _buildValidMessage(messageId, INVEST_VAULT, amount, user, allowedSender);
    ...
    function testSaveInvestCCIP_receiver_ccipReceiveRevertsWhenCallerIsNotRouter() public {
    ...
    function testSaveInvestCCIP_receiver_ccipReceiveRevertsWhenSourceSenderIsNotAllowed() public {
        Client.Any2EVMMessage memory message =
            _buildValidMessage(keccak256("NOT_ALLOWED"), SAVE_VAULT, 10e6, user, makeAddr("notAllowed"));

        vm.prank(router);
        vm.expectRevert(SaveInvestCCIPReceiver.SaveInvestCCIPReceiver__NotAllowedSource.selector);
        receiver.ccipReceive(message);
    }

    function testSaveInvestCCIP_receiver_storesFailedMessageAndRetriesById() public {
        uint256 amount = 15e6;
        ...
        assertEq(uint256(_statusById(messageId)), uint256(SaveInvestCCIPReceiver.StatusCode.FAILED), "expected FAILED");

        sfVault.setShouldRevert(false);
        receiver.retryFailedMessageById(user, messageId);

        assertEq(
            uint256(_statusById(messageId)), uint256(SaveInvestCCIPReceiver.StatusCode.RESOLVED), "expected RESOLVED"
        );
        assertEq(sfVault.lastAssets(), amount, "retry should deposit assets");
    }

    function testSaveInvestCCIP_receiver_recoverTokensByIdTransfersFundsBack() public {
        uint256 amount = 13e6;
        ...
        assertEq(
            uint256(_statusById(messageId)), uint256(SaveInvestCCIPReceiver.StatusCode.RECOVERED), "expected RECOVERED"
        );
    }

    function testSaveInvestCCIP_receiver_retryLatestAndRecoverLatestAcrossMultipleFailedMessages() public {
    ...
    function testSaveInvestCCIP_receiver_malformedPayloadIsCapturedWithoutReverting() public {
    ...
    function testSaveInvestCCIP_receiver_getFailedMessagesOutOfRangePaginationReturnsEmpty() public view {
        SaveInvestCCIPReceiver.FailedMessage[] memory list = receiver.getFailedMessages(100, 10);
        assertEq(list.length, 0, "out of range pagination must return empty array");
    }

    function testSaveInvestCCIP_receiver_retryRevertsWhenDestTokenArrayIsEmpty() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                SaveInvestCCIPReceiver.SaveInvestCCIPReceiver__InvalidDestTokenAmountsLength.selector, 0
            )
        );
        receiver.retryFailedMessageById(user, messageId);
    }

    function testSaveInvestCCIP_receiver_retryRevertsWhenDestTokenAddressIsInvalid() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                SaveInvestCCIPReceiver.SaveInvestCCIPReceiver__InvalidDestTokenAddress.selector, address(notUsdc)
            )
        );
        receiver.retryFailedMessageById(user, messageId);
    }

    function testSaveInvestCCIP_receiver_retryRevertsWhenProtocolSelectorIsInvalid() public {
        ...
        Client.Any2EVMMessage memory message = _buildValidMessage(
            messageId, SAVE_VAULT, amount, allowedSender, abi.encodeWithSignature("mint(uint256,address)", amount, user)
        );
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                SaveInvestCCIPReceiver.SaveInvestCCIPReceiver__InvalidProtocolCallSelector.selector, invalidSelector
            )
        );
        receiver.retryFailedMessageById(user, messageId);
    }

    function testSaveInvestCCIP_receiver_usesProvidedArbitrumSFVaultAddressAndCapturesFailure() public {
        uint256 amount = 6e6;
    ...
    function _statusById(bytes32 messageId) internal view returns (SaveInvestCCIPReceiver.StatusCode status) {
        SaveInvestCCIPReceiver.FailedMessage[] memory list = receiver.getFailedMessages(0, 256);
