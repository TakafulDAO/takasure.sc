...
contract DeploySFVault is Script {
    function run() external returns (HelperConfig.NetworkConfig memory config, SFVault sfVault) {
        ...
        address sfVaultAddress = UnsafeUpgrades.deployUUPSProxy(
            ...
            abi.encodeCall(SFVault.initialize, (IERC20(config.contributionToken), "SF Vault", "SFV"))
        );
