...
1. Run script `01` once the target subgraph is up to date.
2. Set the correct `TOTAL_BACKFILL_TOKENS` in script `02`.
3. Make sure `ADMIN__REVENUE_RECEIVER` is configured in `AddressManager`.
4. Run script `02` and review the allocation outputs.
5. Deploy `ModuleManager`.
6. Deploy `RevShareModule`.
7. Update `deployments/<chain>/ModuleManager.json` and `deployments/<chain>/RevShareModule.json`.
8. Add the required `AddressManager` entries.
9. Ensure `RevShareNFT.setAddressManager(addressManager)` is already set or set it now.
10. Ensure the executor has `OPERATOR`.
11. Fund `RevShareModule` with USDC using `depositNoStream(totalBackfillRaw)`.
12. Run script `03`.
...
2. `node scripts/rev-share-backfill/02-buildRevShareBackfillAllocations.js --chain arb-sep --test`
