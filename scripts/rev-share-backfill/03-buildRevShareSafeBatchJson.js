...
async function proposeToSafe(batches) {
...
async function executeOnSepolia(batches, chainCfg) {
    ...
    for (const batch of batches) {
        ...
        const actualSender = normalizeAddress(receipt.from)
        if (actualSender !== normalizedExpectedExecutor) {
            throw new Error(
                `TESTNET_PK signer mismatch. Expected TESTNET_DEPLOYER_ADDRESS ${normalizedExpectedExecutor}, got ${actualSender}.`,
            )
        }
...
async function main() {
    ...
    console.log(`RevShareModule: ${normalizeAddress(revShareModuleAddress)}`)
    ...
    if (cli.chainId === ARBITRUM_ONE_CHAIN_ID) {
        ...
        actionResult = await proposeToSafe(batches)
        ...
        actionResult = await executeOnSepolia(batches, chainCfg)
    ...
    const report = {
        ...
        safeAddress: actionResult.safeAddress || null,
        proposerAddress: actionResult.proposerAddress || null,
        safeBaseNonce:
            Number.isInteger(actionResult.baseNonce) || typeof actionResult.baseNonce === "number"
                ? actionResult.baseNonce
                : null,
        txServiceUrl: actionResult.txServiceUrl || null,
        executorAddress: actionResult.executorAddress || null,
        proposals: actionResult.proposals || [],
    ...
    fs.writeFileSync(outputPaths.reportJson, JSON.stringify(report, null, 2), "utf8")
