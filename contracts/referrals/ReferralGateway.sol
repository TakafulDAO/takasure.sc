    ...
    PausableUpgradeable
{
    ...
    function _payContribution(
        ...
        if (_isModifying) {
            ...
            // The formula will be _planToApply * (YEAR - (block.timestamp - associationTimestamp)) / YEAR
            ...
            proratedContribution =
                (_prorateOneYear(normalizedContribution, _associationTimestamp) /
                    DECIMAL_REQUIREMENT_PRECISION_USDC) *
                DECIMAL_REQUIREMENT_PRECISION_USDC;
        } else {
    ...
    function _checksIfModifying(
        ...
        if (_couponAmount > 0)
            require(_couponAmount <= MAXIMUM_CONTRIBUTION, ReferralGateway__InvalidContribution());
